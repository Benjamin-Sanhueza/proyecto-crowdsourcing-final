...
export const upsertActiveStudents = async (req: Request, res: Response) => {
  ...
  try {
    // IMPORTANTE: La tabla se llama 'monthly_active_students' según nuestro último script
...
export const multiRegressionDataset = async (req: Request, res: Response) => {
  const allowed = ['equipment', 'infrastructure', 'services', 'other'];
  ...
  try {
    const result = await db.query(
      ...
      SELECT
        w.id,
        w.user_id,
        w.category,                                     -- Ya es texto, no casteamos
        COALESCE(w.prev_category, 'none') AS prev_category,
        w.prior_count,
        u.education_level,                              -- Ya es texto
        ...
        -- AQUÍ ESTABA EL ERROR: Quitamos el ::incident_category
        CASE WHEN w.category = $1 THEN 1 ELSE 0 END AS y
      FROM w
    ...
    console.error('Error MultiRegression:', e);
